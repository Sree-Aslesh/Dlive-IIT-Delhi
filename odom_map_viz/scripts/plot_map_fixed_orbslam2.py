...
class  MapViz(object):
	def __init__(self):
		...
		self.last_odom2_x = None
		self.last_odom2_y = None
		...
		#By lokesh start
		#self.vo_odom_sub_t =  rospy.Subscriber('/'+odom_topic_t, Vector3, self.odom_cb_2, queue_size=1)
		#self.vo_odom_sub_q =  rospy.Subscriber('/'+odom_topic_q, Quaternion, self.odom_cb_3, queue_size=1)
		#by lokesh end
		...
		self.initial_pose_n = Quaternion(imu_x,imu_y,imu_z,imu_w).normalised
		self.q = Quaternion(axis=[0,0,1], angle=np.pi).normalised

		self.initial_pose = ((self.initial_pose_n.inverse)* self.q).normalised
		...
		rospy.loginfo("GPS signals received")
	def odom_cb(self, data):
	 ...
	 #BY lokesh start
	#def odom_cb_2(self, data):
		#print "in if block"
		#self.last_odom2_x = data.position.x
		#self.last_odom2_y = data.position.y
		
	#def odom_cb_3(self, data):
		#print "in if block"
		#self.vo_orientation2 = Quaternion(data.x, data.y, data.z, data.w ).inverse
	...
	def do_work(self):
		...
		if self.offset_x +self.last_odom_x >=0 and self.offset_x+ self.last_odom_x < 7164 and self.offset_y+self.last_odom_y>=0 and self.offset_y+self.last_odom_y< 3358:
			...
			#By lokesh	
			#corrected_coordinates_2 = self.initial_pose.rotate([self.last_odom2_x, self.last_odom2_y, 0])
			#cv2.circle(self.map_img,(int(self.offset_x+corrected_coordinates_2[0]),int(self.offset_y+corrected_coordinates_2[1])),1,(255,0,),-1)	
